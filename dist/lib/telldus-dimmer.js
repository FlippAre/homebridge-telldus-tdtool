...
class TelldusDimmer extends TelldusAccessory {
  ...
  constructor(data, log, homebridge, config) {
    ...
    this.service.getCharacteristic(this.Characteristic.On).on('get', this.getOnState.bind(this)).on('set', this.setOnState.bind(this));

    this.service.getCharacteristic(this.Characteristic.Brightness).on('get', this.getDimState.bind(this)).on('set', this.setDimState.bind(this));
  ...
  getDimState(callback) {
    ...
    this.getState((err, state) => {
      ...
      if (state.name == 'OFF') {
        ...
        this.log("Lightbulb is on, brightness: " + bitsToPercentage(state.level) + "%");
