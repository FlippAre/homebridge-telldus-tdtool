...
class TelldusDoor extends TelldusAccessory {
  ...
  constructor(data, log, homebridge, config) {
    ...
    this.service.getCharacteristic(this.Characteristic.CurrentPosition).on('get', this.getCurrentPositionState.bind(this));

    this.service.getCharacteristic(this.Characteristic.CurrentDoorState).on('get', this.getOpenState.bind(this));

    this.service.getCharacteristic(this.Characteristic.PositionState).on('get', this.getPositionState.bind(this));

    this.meta.setCharacteristic(this.Characteristic.Model, "Door");
